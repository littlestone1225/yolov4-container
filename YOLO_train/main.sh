...
echo "$YOLO_DARKNET_PATH"/darknet detector train "$YOLO_CONFIG_PATH"/"$PROJECT_NAME".data "$YOLO_CONFIG_PATH"/"$PROJECT_NAME".cfg "$YOLOV4_BEST_MODEL" -dont_show -clear 1 -map > /dev/null 2>&1& #-gpus 1 
"$YOLO_DARKNET_PATH"/darknet detector train "$YOLO_CONFIG_PATH"/"$PROJECT_NAME".data "$YOLO_CONFIG_PATH"/"$PROJECT_NAME".cfg "$YOLOV4_BEST_MODEL" -dont_show -clear 1 -map > /dev/null 2>&1& #-gpus 1 
